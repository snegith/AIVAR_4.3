...
- PR: ruff, mypy, pytest (three detectors in isolation + scorer + API), then an explicit `simulate.py` step against a live uvicorn + Postgres service container (health-gated via `/ready` before and after pytest), all with `LANGFUSE_ENABLED=false` and `--dry-run`. `test_simulate.py` fails (not skips) in CI when the API is down.
...
- Phase 10 - Containerize & CI. `Dockerfile.api`, `ci.yml` (lint/type/tests/simulate with dry-run, Langfuse off). Test: CI green on PR. Depends on: Phases 7-8.
  - **CI simulate gate (mandatory):** `ci.yml` starts uvicorn as a background process, blocks until `GET /ready` returns 200, runs pytest, re-checks `/ready` immediately before `simulate.py`, then runs `python simulate.py --base-url … --dry-run` as its own step (not only the pytest subprocess wrapper). `test_simulate.py` must **fail** (not skip) when `CI=true` and the API is unreachable. The explicit simulate step is the authoritative end-to-end gate.
- Phase 11 - AWS deploy (free tier). `ec2_bootstrap.sh` (Docker + 2GB swap), `deploy.sh`, `migrate.sh`; EC2 t3.micro + RDS db.t3.micro (`detector_db` + `langfuse_db`); `docker-compose.ec2.yml` runs api + Langfuse v2. Test: EC2 `:8000/health` returns ok; a live `POST /v1/events` works against RDS; the Langfuse UI loads at `:3000` and shows mirrored traces; `simulate.py --base-url http://EC2_IP:8000` passes. Depends on: Phase 10.
  - **Post-deploy ordering (mandatory — do this before anything else):** Immediately after `docker compose up` + migrations, **before** Langfuse UI checks, demo prep, or README smoke tests, run `simulate.py --base-url http://EC2_IP:8000` against the live RDS-backed API. This is the first validation step on every deploy.
  - **Persona C RDS gate (carry-forward from Phase 8):** Persona C (normal high-volume user) is the false-positive risk on AWS/RDS — latency and BackgroundTask timing on RDS can push `risk_score` higher than local runs. Inspect the simulate results table: Persona C `risk_score` must stay **below `WATCH_THRESHOLD` (45.0)** with comfortable margin. If C scores ≥ 45.0 on RDS, **stop and tune only config-driven thresholds/weights** (no detector logic changes) before proceeding with Langfuse, demo script, or phase sign-off. After Phase 8 fixup, local seed-42 runs target a wider margin than the original ~0.93-point headroom; RDS must be re-verified because remote DB latency is not replicated in CI.
  - **`deploy/post_deploy_smoke.sh`:** Wrap the above — wait for `/ready`, run full `simulate.py`, grep/parse Persona C score, exit non-zero if C ≥ 45 or any persona fails. `deploy.sh` calls this script as its final step.
